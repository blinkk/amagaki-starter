import {DataType, Document, Environment} from '@amagaki/amagaki';
...
export class Url {
  ...
  static relative(other: Document | string, base: Document | string) {
    const otherUrl = (
      DataType.isDocument(other) ? (other as Document).url?.path : other
    ) as string;
    const baseUrl = (
      DataType.isDocument(base) ? (base as Document).url?.path : base
    ) as string;
